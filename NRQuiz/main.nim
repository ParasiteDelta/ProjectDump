...
proc makeWorksheet(op: string) =
    ...
    while lnum < num_prob:
        ...
        while i < 1:
            ...
            if num1 in (num2 - sepPerc..num2 + sepPerc):
                num1 = rand(num_low..num_high)
                continue
            if num2 in (num1 - sepPerc..num1 + sepPerc):
                num2 = rand(num_low..num_high)
                continue
            if num1 in (n1Last - sepPerc..n1Last + sepPerc):
                num1 = rand(num_low..num_high)
                continue
            if num2 in (n2Last - sepPerc..n2Last + sepPerc):
                num2 = rand(num_low..num_high)
                continue
            if numSum in (nsLast - sepPerc..nsLast + sepPerc):
                num1 = rand(num_low..num_high)
                num2 = rand(num_low..num_high)
                continue
            if num1 in usedNums:
